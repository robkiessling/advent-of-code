...
const CARD_PRIORITY = 'AKQT98765432J';
...
function getHandType(hand) {
  ...
  // If there are jokers, convert them to be whatever card is the most common
  // Note: if 2 cards are the most common (e.g. in '4455J'), it doesn't matter which one you add it to, it will always be a triple
  if (cardCounts['J'] && cardCounts['J'] !== 5) {
    let mostCommonCard;

    for (const [cardType, cardCount] of Object.entries(cardCounts)) {
      if (cardType === 'J') { continue; }
      if (mostCommonCard === undefined || (cardCount > cardCounts[mostCommonCard])) {
        mostCommonCard = cardType;
      }
    }
...
console.log(getTotalWinnings(readFile('./example.txt'))); // expected: 5905
console.log(getTotalWinnings(readFile('./puzzle.txt'))); // expected: 248781813
